#include <Servo.h>    // inclui biblioteca de manipulação de servos motores.    
  ...
  AF_DCMotor motor3(3);    // Define o motor1 ligado ao M1  
  AF_DCMotor motor4(4);    // Define o motor2 ligado ao M2  
  ...
  int velocidadeMotores = 600; // velocidade que os motores funcionarão na bateria 9v. Para a bateria 9v a velocidade 80 é ideal
  ...
  void setup(){    
    ...
    motor3.setSpeed(velocidadeMotores);     // Define a velocidade para os motores. A velocidade máxima é 255. 
    motor4.setSpeed(velocidadeMotores);
  ...
  {    
   ...
   for (int i=0; i <= 3; i++){
      ...
      motor2.run(BACKWARD); 
      motor3.run(BACKWARD);    // Roda vai para trás  
  ...
  {    
   ...
   motor2.run(BACKWARD); 
   motor3.run(BACKWARD);    //o robô dá uma ré para não colidir ao girar 
   motor4.run(BACKWARD);       
   delay(50);  
   digitalWrite(BUZZER, LOW); // Desliga o som  
   delay(100);
